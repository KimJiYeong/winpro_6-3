...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam) //CALLBACK(윈도우 핸들, 처리해야 할 메시지의 값,입력장치의 값1,2)
{
	...
	case WM_PAINT:
		...
		if (Spring) {
			...
			//	LineTo(hdc, 0 - move_xx, 400);
			//sin 그래프를 사용해서 주기마다 체크한다.

			for (f = -100; f < 400; f += 0.1) {
				spring_y = (int)(sin(f*3.14 / 50 + move_xx / 10) * (90 + move_yy)) + 400;
				spring_x = (int)(cos(f*3.14 / 50 + move_xx / 10) * (90 - move_yy)) + 400;
				//진폭
				//SetPixel(hdc, (int)f + spring_x , spring_y, RGB(color_R, color_G, color_B));
				LineTo(hdc, f + spring_x, spring_y);
				//	spring_x +=;
